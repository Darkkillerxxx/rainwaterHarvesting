...
app.get("/getAllLocationForDistricts", async (req, res) => {
  try {
    ...
    const response = await queryData(
      `SELECT Latitude,longitude,Village from Water_Harvesting WHERE DISTRICT='${DISTRICT}' AND Latitude IS NOT NULL AND Longitude IS NOT NULL`,
    );
...
app.get("/getAllDistrics", async (req, res) => {
  const response = await queryData(
    `select Distinct DISTRICT from Water_Harvesting`,
  );
  res.send({
    code: 200,
    message: "Success",
...
app.get("/getPicklistValues", async (req, res) => {
  try {
    const response = await queryData(
      `select Distinct DISTRICT,TALUKA,VILLAGE from Water_Harvesting`,
    );
    res.send({
      code: 200,
      message: "Success",
...
app.get("/fetchRecords", async (req, res) => {
  try {
    ...
    console.log(
      `SELECT * FROM Water_Harvesting WHERE TALUKA='${Taluka}' ORDER BY ID OFFSET ${offSet ? offSet : 0} ROWS FETCH NEXT 11 ROWS ONLY`,
    );
